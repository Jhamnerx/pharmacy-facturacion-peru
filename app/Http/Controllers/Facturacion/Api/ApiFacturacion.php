...
class ApiFacturacion extends Controller
{
    ...
    public function emitirNotaCredito(Comprobantes $nota, $tipo_comprobante)
    {
        ...
        $items = $this->getItemsInvoice($nota->venta->detalle);
    ...
    public function createObjetInvoice(Ventas $venta, $tipo_operacion)
    {
        ...
        if ($venta->anticipos->count() > 0) {
            //ESTABLECER VALORES
            $invoice->setValorVenta($venta->detalle->sum('sub_total'))
                ->setSubTotal($venta->detalle->sum('total'));
        ...
        //ESTABLECER ITEMS DEL COMPROBANTE
        $items = $this->getItemsInvoice($venta->detalle);
    ...
    public function sendInvoiceOnlyNota(Comprobantes $nota)
    {
        ...
        $result = $see->sendXml(get_class($nota->clase), $nota->clase->getName(), Storage::disk('facturacion')->get('/xml' . '/' . $nota->nombre_xml . '.xml'));
    ...
    public function sendInvoiceOnlyGuia($guia)
    {
        ...
        $res = $api->sendXml($guia->clase->getName(), Storage::disk('facturacion')->get('/xml' . '/' . $guia->nombre_xml . '.xml'));
    ...
    public function emitirGuia(GuiaRemision $guia)
    {
        ...
        $envio
            ...
            ->setUndPesoTotal('KGM')
            ->setLlegada($this->getDirectionGuiaLLegada($guia, $empresa->ruc))

            ->setPartida($this->getDirectionGuiaPartida($guia, $empresa->ruc));
