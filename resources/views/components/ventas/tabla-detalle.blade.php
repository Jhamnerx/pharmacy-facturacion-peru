    ...
    <table class="w-full">
        ...
        <tbody class="text-xs divide-y divide-slate-200 listaItems dark:divide-slate-600">
            @foreach ($prepayments->all() as $clave => $prepayment)
                <tr class="main bg-slate-100 dark:bg-slate-800" wire:key="item-prepay-{{ $clave }}">
            ...
            @foreach ($items->all() as $clave => $item)
                <tr class="main bg-slate-100 dark:bg-slate-800" wire:key="item-{{ $clave }}">
                    <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                    ...
                    <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap min-w-36 md:min-w-0">
                        ...
                        <x-form.number wire:model.live="items.{{ $clave }}.cantidad" min="1"
                    ...
                    <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                        <div class="font-normal text-center">
                            {{ $items[$clave]['sub_total'] }}
                        </div>

                        @if ($errors->has('items.' . $clave . '.sub_total'))
                            <p class="mt-2  text-pink-600 text-sm">
                                {{ $errors->first('items.' . $clave . '.sub_total') }}
                            </p>
                        @endif
                    </td>
